...
5. **Try to break it.** Trace the main flows end to end. Write throwaway tests in a scratch directory, never inside the repo. Launch the app (via the `run` skill if available, otherwise the documented command) against a local or seeded environment and walk the main flows once as a QA pass: happy path, one invalid input, one interrupted step per flow, watching console and network for errors. Record each flow as passed, failed, or not testable and why. Skip the launch only if no credential-free local way exists. Never touch external services, never use credentials. Record every attempt and its outcome.
